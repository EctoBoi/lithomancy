import {
  GameState, Hand, StoneFace, CastResult, CastType,
  Shape, SHAPE_SIDES, ServerMessage, ClientMessage,
  classifyCast,
} from '../shared/game';
...
function setStatus(t: string) { statusBar.textContent = t; }
...
function turretPos(i: number): [number, number] {
  ...
  return [
    CX + (TOWER_R + TURRET_R + 10) * Math.cos(angle),
    CY + (TOWER_R + TURRET_R + 10) * Math.sin(angle),
  ];
}
...
function castLabel(t: CastType): string {
  switch (t) {
    ...
    case 'full_potion': return '⚗️ Full Potion';
    case 'regular_potion': return '⚗️ Potion';
    case 'full_charm': return '🔮 Full Charm';
    case 'regular_charm': return '🔮 Charm';
    case 'bungle': return '💨 Bungle';
  }
}
...
function render() {
  ...
  let phaseText = '';
  switch (s.phase) {
    case 'waiting': phaseText = 'Waiting for opponent…'; break;
    case 'casting':
      phaseText = s.castReady[playerIndex] ? 'Waiting for opponent to cast…' : 'Cast your stones!';
      break;
    case 'recast':
      phaseText = s.recastDecision[playerIndex] !== null
        ? 'Waiting for opponent…'
        : 'Recast? Select stones to reroll, then decide.';
      break;
    case 'reveal': phaseText = s.lastOutcome?.reason ?? ''; break;
    case 'action':
      phaseText = s.activePlayer === playerIndex
  ...
  if (s.lastOutcome && s.phase === 'reveal') {
    outcomeBanner.textContent = s.lastOutcome.winner === 'draw'
      ? 'Draw — replaying!'
...
type ActionMode =
  ...
  | { mode: 'spell' }
  | { mode: 'charm' }
  | { mode: 'potion_my'; myTurret: number | null }
...
function handleTurretClick(i: number) {
  ...
  if (actionMode.mode === 'spell') {
    if (s.board[i] !== null) { setStatus('That turret is occupied.'); return; }
    send({ type: 'action_spell', turretIndex: i });
    actionMode = { mode: 'none' };
    updateUI();
  } else if (actionMode.mode === 'charm') {
    if (s.board[i] !== opponent) { setStatus("Pick an opponent's turret gem."); return; }
    ...
    if (actionMode.myTurret === null) {
      // pick own gem
      if (s.board[i] !== playerIndex) { setStatus('Pick one of your turret gems first.'); return; }
      actionMode = { mode: 'potion_my', myTurret: i };
      setStatus('Now click an adjacent opponent turret gem.');
    } else {
      // pick opponent adjacent gem
      if (s.board[i] !== opponent) { setStatus("Pick an opponent's gem."); return; }
